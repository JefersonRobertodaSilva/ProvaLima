...
Bloco* ImprimirListaRecursivo(Bloco* inicio){
	Bloco* aux= inicio;
	...
	return ImprimirListaRecursivo(aux->prox);
}


Bloco* BuscaRecursivo(Bloco* inicio, int valor){
	Bloco* aux=inicio;
	...
	if(aux==NULL){
		...
		return BuscaRecursivo(aux->prox, valor);
	}
}

//INCOMPLETA
Bloco* ExcluirElementoRecursiva(Bloco** inicio, Bloco* anterior, int valor){
	  if (*inicio == NULL) { 
	...
	return ExcluirElementoRecursiva(&aux->prox, valor);
